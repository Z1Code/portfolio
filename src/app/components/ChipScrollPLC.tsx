...
export default function ChipScrollPLC() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= TOTAL_FRAMES; i++) {
      const img = new Image();
      img.src = `/sequence3/ezgif-frame-${String(i).padStart(3, "0")}.jpg`;
        ...
        {isLoading && (
          ...
          >
            <div className="relative h-1 w-32 overflow-hidden rounded-full bg-white/10">
              <motion.div
                className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-cyan-400 via-indigo-500 to-purple-500"
                initial={{ width: 0 }}
